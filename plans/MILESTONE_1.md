...
- `sync` discovers bounded non-dot directories containing `SKILL.md`, validates the declarative skill subset, rejects command-enabling metadata, detects case-insensitive collisions, renders stable per-target output, copies supporting files as non-executable data, installs atomically, writes source/skill/target ownership markers, and prunes only validly marked stale output on enabled targets.
