...
Skills and agents files can share instructions while varying parts by harness. A whole-line `{{esheep.include-by-harness "body"}}` inserts `esheep-body-pi.md`, `esheep-body-codex.md`, or `esheep-body-claude.md` from the skill directory or the source's `agents-md/` directory. Use `{{esheep.include-by-harness-optional "extras"}}` when an absent harness file should insert zero bytes without fallback. Both directives expand recursively from the same owning root, with cycle detection and a two-level nesting limit. Agents files use the same variable language throughout, without interpreted frontmatter; content outside substitutions stays byte-identical. Include fragments remain source-only. See `esheep help skill-format` for the complete include contract.
